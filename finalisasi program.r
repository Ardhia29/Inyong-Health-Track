...
# Fungsi untuk menghitung risiko keturunan
...
# Data koordinat rumah sakit (latitude dan longitude)
...
# Data tenaga medis (dengan struktur hierarkis dan koordinat)
...
# Fungsi untuk mendapatkan daftar fasilitas kesehatan berdasarkan kota
...
# Fungsi untuk mendapatkan daftar dokter berdasarkan fasilitas kesehatan
...
# Fungsi untuk mendapatkan koordinat fasilitas kesehatan
...
ui <- dashboardPage(
  ...
  dashboardSidebar(
    ...
    sidebarMenu(
      ...
      menuItem("⚙️ Pengaturan", tabName = "settings", icon = icon("gear")),
      div(
      ...
      tabItem(
        ...
        fluidRow(
          box(
            ...
            textInput("name", "Nama Lengkap:", placeholder = "Contoh: Budi Santoso"),
      ...
      tabItem(
        ...
        fluidRow(
          box(
            ...
            selectInput("theme", "Tema Warna:", choices = c("Default", "Gelap", "Terang")),
...
server <- function(input, output, session) {
  ...
  # Output: Rekomendasi Gizi
  output$nutrition_rec <- renderUI({
    rec <- c()
    
    # Evaluasi karbohidrat
  ...
  # Output: Informasi Risiko Keturunan
  output$family_risk_info <- renderUI({
    req(input$submit_family)
  ...
  # Output: UI untuk memilih fasilitas kesehatan
  ...
  # Output: UI untuk menampilkan daftar dokter dengan tombol Google Maps
  output$doctors_ui <- renderUI({
    ...
    if (input$city_select != "Pilih kota..." && input$facility_select != "Pilih fasilitas...") {
      ...
      if (!is.null(doctors) && length(doctors) > 0) {
        ...
        map_url <- paste0("https://www.google.com/maps?q=", 
                         coords$lat, ",", coords$lng,
                         "&hl=id&z=17")
        
        # Tambahkan tombol Google Maps di atas daftar dokter
        div(
          # Tombol Google Maps
  ...
  # Event untuk tombol submit pertanyaan
